...
void LBPH::save(string filename ) const {
	//fs << "radius" << _radius;
	//fs << "neighbors" << _neighbors;
	//fs << "grid_x" << _grid_x;
	//fs << "grid_y" << _grid_y;
	// write matrices
	//writeFileNodeList(fs, "TestImages", _histograms);
	FileStorage fs1(filename, FileStorage::WRITE);
	fs1 << "TestImages" << _histograms;
	const int *p = _labels.ptr<int>(0); // pointer to row 0
	std::vector<int> vec(p, p+_labels.rows); // construct a vector using pointer
	fs1 << "LabelImages" << vec;
	fs1.release();
	//fs << "uniform" << _uniform;
}
...
void LBPH::predict(InputArray _src, int &minClass, double &minDist) const {
	...
	for(size_t sampleIdx = 0; sampleIdx < _histograms.size(); sampleIdx++) {
		double dist = compareHist(_histograms[sampleIdx], query, 1);
